...
namespace ASC.Web.Host.HttpRequestProcessor
{
    class Request : SimpleWorkerRequest
    {
        ...
        public ArrayList DefaultDocuments
        {
            get
            {
                ArrayList list = new ArrayList(RequestConfiguration.DefaultFileNames);
                if (_host.DefaultDocument.Length > 0)
                {
                    list.Insert(0, _host.DefaultDocument);
                }
                return list;
            }
        }

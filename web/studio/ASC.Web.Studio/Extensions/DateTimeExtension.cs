...
namespace System
{
    public static class DateTimeExtension
    {
        public static string DateMaskForJQuery
        {
            get
            {
                return System.Threading.Thread.CurrentThread.CurrentCulture.DateTimeFormat.ShortDatePattern.Replace("dd", "99").Replace("d", "99").Replace("MM", "99").Replace("M", "99").Replace("y", "9").Replace("Y", "9");
            }
        }
        ...
        {
            DateTime temp = target.Date;
            temp = temp.AddYears(from.Date.Year - temp.Year);
            TimeSpan diff = (temp - from.Date);
        ...
        {
            StringBuilder result = new StringBuilder();
            TimeSpan diff = (TenantUtil.DateTimeNow().Date - target.Date);
        ...
        public static string AgoSentence(this DateTime target)
        {
            ...
            bool isYesterdayOrDayBefore = false;

            StringBuilder result = new StringBuilder();
            TimeSpan diff = (TenantUtil.DateTimeNow() - target);

            int years = diff.Days / yearDuration;
            int months = diff.Days / monthDuration;

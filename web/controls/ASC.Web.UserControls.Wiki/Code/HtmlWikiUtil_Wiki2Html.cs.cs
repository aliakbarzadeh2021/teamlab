...
namespace ASC.Web.UserControls.Wiki
{
    ...
    public partial class HtmlWikiUtil
    {
        ...
        private static string _urlChars = @"a-z0-9;/\?:@&=\+\$,\-_\.!~\*'\(\)#%\\"; // a-z0-9;/?:@&=+$,-_.!~*'()#%\
        private static string _urlEndChars = @"a-z0-9;/\?:@&=\+\$\-_\\\)";            // a-z0-9;/?:@&=+$-_\
        ...
        private static Match[] GetWikiSectionMatches(string wiki)
        {
            ...
            //return (new List<Match>(allSectionMatches.Cast<Match>())).ToArray();
        }
        ...
        private void Init(string pageName, string intMainLink, List<string> existPages, string imgHandlerUrlFormat, int tenant, ConvertType isEditable, int thWidth)
        {
            ...
            //noWikiList = new List<NoWikiContainer>();
        ...
        private string Ref(string sUrl, string sTxt, string sTgt, string all)
        {
            ...
            else //link
            {
                ...
                {
                    result = string.Format(@"{0} title=""{1}""", result, sUrl);
                }
        ...
        private string ProcessSavePreCollection(string s)
        {
            ...
            {
                container = new TempContainer() { Content = m.Value.Replace(m.Groups[1].Value.Trim(), XLT(m.Groups[1].Value.Trim(), XltType.Pre)) };
        ...
        private string ProcessSpecCategoryLinks(string source)
        {
            ...
            Regex catReg = new Regex(PagesProvider.RegExCategorySearch, RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
                ...
                else //Parse the line
                {
                    ...
                    if ((xltType & XltType.Table) == XltType.Normal)
                    {
                        ...
                        foreach (Match match in Matches)
                        {
                            ...
                            {
                                s = Replace(s, match.Groups[4].Value, Ref("mailto:", match.Groups[4].Value, match.Groups[4].Value, match.Groups[4].Value), 0, 1);
                            }
                    ...
                    if ((xltType & XltType.Table) == XltType.Normal)
                    {
                        ...
                        foreach (Match match in Matches)
                        {
                            string replaceWidth = string.Empty;
        ...
        private static string HighLightCode(string source)
        {
            ...
            MatchCollection mcCode = rexCode.Matches(source);
            string sCode;
            foreach (Match m in mcCode)
            {
                try
                {
                    sCode = m.Groups[1].Value.Substring(m.Groups[1].Value.IndexOf(">") + 1);
        ...
        private static string wikiLineSymbols = @"<< >> <> >< ! {| |} |+ |- | ^ * # ; = - ~~";


        private static bool isStringInternalWiki(string str)
        {
            string ss = str.Trim();
            foreach (string s in wikiLineSymbols.Split(" ".ToCharArray(), StringSplitOptions.RemoveEmptyEntries))
            {
                if (ss.StartsWith(s))
                {
                    return true;

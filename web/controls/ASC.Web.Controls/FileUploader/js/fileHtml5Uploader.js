...
var FileHtml5Uploader = new function() {
    ...
    this.InitFileUploader = function(conf) {
        ...
        if (FileHtml5Uploader.EnableHtml5) {
            ...
            if (SWFUploadObject.length != 0)
                SWFUploadObject.css('top', -jq("#SWFUpload_" + (SWFUpload.movieCount - 1)).offset().top);
        }
    ...
    this.InitHtml5Uploader = function() {
        ...
        setTimeout(function() {
            ...
            jq('#' + FileHtml5Uploader.config.UploadButtonID).html("<div id='fu_multi_container' style='overflow: hidden; width: 128px; height: 20px; cursor: pointer;'></div><a id='fu_button_select' class='grayLinkButton' href='javascript:void(0);' style='width: 105px; margin: -20px 0 5px 0; cursor: pointer;'>" + buttonText + "</a>");
            ...
            if (FileHtml5Uploader.config.OverAllProcessHolder != null) {
                jq(FileHtml5Uploader.config.OverAllProcessHolder).html("<table cellpadding='5' cellspacing='0' width='100%' style='padding:10px 0;' class='describeText'><tr><td width='100'>" + FileHtml5Uploader.config.OverallProgressText + ": </td><td>" + jq.format('<div style="margin-top: 1px;" class="{0}"><div></div></div>', FileHtml5Uploader.config.OverAllProcessBarCssClass) + "</td><td class='fu_percent' width='20'></td></tr></table>");
    ...
    this.OnMultipleInputChange = function(files) {
        ...
        if (FileHtml5Uploader.config.FilesHeaderCountHolder != null && FileHtml5Uploader.FilesToUpload.length > 0)
            jq(FileHtml5Uploader.config.FilesHeaderCountHolder).text(jq.format(FileHtml5Uploader.config.SelectedFilesText, FileHtml5Uploader.FilesToUpload.length));
    ...
    this.RenderItemToUpload = function(data, exceedLimit) {
        ...
        if (data.ReturnCode && data.ReturnCode == 1) {
            ...
            // should be implemented in GlobalConfig for common using
            /*|| ASC.Controls.FileUploaderGlobalConfig.ErrorFileTypeText*/
            sb += FileHtml5Uploader.config.ErrorFileTypeText;
        }
        ...
        var dataSize = data.size || data.fileSize;
    ...
    this.CheckFileSize = function(file) {
        ...
        if (size == 0 || size > FileHtml5Uploader.config.FileSizeLimit * 1024) {
            return false;
        }
        return true;
    };
    ...
    this.TransferComplete = function(evt, file, xhr) {
        ...
        if (FileHtml5Uploader.config.FilesHeaderCountHolder != null) {
            jq(FileHtml5Uploader.config.FilesHeaderCountHolder).text(jq.format(FileHtml5Uploader.config.FilesHeaderText, FileHtml5Uploader.UploadedFilesCount, FileHtml5Uploader.GetUploadFileCount()));
        }
    ...
    this.HideDragHighlight = function() {
        if (FileHtml5Uploader.config.DragDropHolder != null)
            jq(FileHtml5Uploader.config.DragDropHolder).css({
                "border-style": "solid",
                "border-color": "#d1d1d1",

...
ASC.Controls.FileUploader = function(config) {
    ...
    var _onUploadComplete = function(data, serverData) {
        ...
        if (config.FilesHeaderCountHolder != null) {
            jq(config.FilesHeaderCountHolder).text(jq.format(config.FilesHeaderText, FileUploader.UploadedFilesCount, FileUploader.GetTotalFileCount()));
        }
    ...
    var _renderUploadItem = function(data) {
        ...
        //size
        sb += '<td style="padding-left: 10px; width:20%; color: #83888D;">'//.format(false /*exceedLimit*/ ? 'color: red;' : 'color: #83888D;');
    ...
    if (config.OverAllProcessHolder != null) {
        jq(config.OverAllProcessHolder).html("<table cellpadding='5' cellspacing='0' width='100%' style='padding:10px 0;' class='describeText'><tr><td width='100'>" + config.OverallProgressText + ": </td><td>" + jq.format('<div style="margin-top: 1px;" class="{0}"><div></div></div>', config.OverAllProcessBarCssClass) + "</td><td class='fu_percent' width='20'></td></tr></table>");
    ...
    if (this.IsSWF) {
        ...
        if (config.SelectFilesText != null && config.SelectFilesText != "")
            uploadButton.text(config.SelectFilesText)
        ...
        swfuploader = new SWFUpload({
            ...
            file_size_limit: (config.FileSizeLimit != null && config.FileSizeLimit != 0) ? config.FileSizeLimit : "0",
            ...
            file_dialog_complete_handler: this.AutoSubmit ? function() { this.startUpload() } : null,
            // Button settings
        ...
        this.SetFormData = function(scope) {
            ...
            if (config.SelectFilesText != null && config.SelectFilesText != "")
                jq(newButton).text(config.SelectFilesText);
            ...
            var au = new AjaxUpload(scope.ButtonID, {
                ...
                onComplete: function(file, response) {
                    ...
                    _onUploadComplete({ UploadId: this.Utx,
                        CurrentFile: file,
                        TotalBytes: 0,
                        Progress: 0,
                        HasDelete: true,
        ...
        this.OnMonitor = function(scope) {
            var callback = function(data) {
                ...
                if (data.IsFinished == true) {
                    ...
                    _onProgress(data);
                    setTimeout(function() { o.OnMonitor(o); }, o.RequestTimeSpan);
                }
            };
    ...
    this.HandlerFileSelected = function(increase) {
        ...
        if (config.FilesHeaderCountHolder != null && this.GetTotalFileCount() > 0)
            jq(config.FilesHeaderCountHolder).text(jq.format(config.SelectedFilesText, this.GetTotalFileCount()));

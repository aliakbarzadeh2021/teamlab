...
drop index core_group_parentid;
drop index core_groupmanager_tenant;
drop index core_subscription_core_subscription_uix;
drop index core_user_index_core_user_username;
drop index core_usergroup_tenant;
drop index core_usersecurity_pwdhash;
...
insert into core_group select tenant, id, name, categoryid, parentid, 0, datetime('now') from core_group_tmp;
...
insert into core_user select tenant, id, username, firstname, lastname, sex, bithdate, status, 1, email, workfromdate, terminateddate, title, department, contacts, location, notes, 0, datetime('now') from core_user_tmp;
insert into core_usergroup select tenant, userid, groupid, 0, 0, datetime('now') from core_usergroup_tmp;
insert into core_usergroup select tenant, userid, groupid, 1, 0, datetime('now') from core_groupmanager;
...
insert into tenants_quotarow select tenant, path, counter, tag, datetime('now') from tenants_quotarow_tmp;
insert into tenants_tenants select id, name, alias, mappeddomain, language, timezone, trusteddomains, trusteddomainsenabled, status, statuschanged, creationdatetime, null, 0, null, datetime('now') from tenants_tenants_tmp;

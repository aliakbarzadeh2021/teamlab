    ...
    public class SqlQuery : ISqlInstruction
    {
        ...
        public string ToString(ISqlDialect dialect)
        {
            ...
            if (0 < groups.Count)
            {
                ...
                {
                    sql.AppendFormat("{0}, ", group is string ? dialect.Bracket(group.ToString()) : group.ToString());
                }
                sql.Remove(sql.Length - 2, 2);
            }
            ...
            if (0 < orders.Count)
            {
                ...
                {
                    sql.Append(order.Key is string ? dialect.Bracket(order.Key.ToString()) : order.Key.ToString());
                    sql.AppendFormat(" {0}, ", order.Value ? "asc" : "desc");
                }
                sql.Remove(sql.Length - 2, 2);
            }

            dialect.SetPagging(sql, maxResults, firstResult);
        ...
        {
            get { return new SqlInstructionDebuggerDisplay(this); }
        }
    }
}
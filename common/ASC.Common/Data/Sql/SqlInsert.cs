...
namespace ASC.Common.Data.Sql
{
    public class SqlInsert : ISqlInstruction
    {
        ...
        public string ToString(ISqlDialect dialect)
        {
            ...
            sql.AppendFormat("{0} into {1}", replaceExists ? dialect.InsertOrReplace : "insert", dialect.Bracket(table));
            ...
            if (0 < columns.Count)
            {
                ...
                for (int i = 0; i < columns.Count; i++)
                {
                    ...
                    sql.AppendFormat("{0}, ", dialect.Bracket(columns[i]));
                }
            ...
            if (returnIdentity)
            {
                sql.AppendFormat("; select {0}", identityInsert ? dialect.GetIdentityQuery : "?");
            }
            return sql.ToString();
        }

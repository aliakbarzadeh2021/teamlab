...
namespace ASC.Common.Utils
{
    public class HtmlUtil
    {
        ...
        private static readonly Regex BrReplacer = new Regex(@"<br\s*\/*>", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.IgnorePatternWhitespace | RegexOptions.Compiled);
        private static readonly Regex TagReplacer = new Regex(@"<\s*(style|script)[^>]*>(.*?)<\s*/\s*(style|script)>", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.Compiled | RegexOptions.Multiline);
		...
		public static string GetText(string html, int maxLength, string endBlockTemplate, bool calcEndBlockTemplate, bool preserveSpaces)
		{
            ...
            //Kill all malicious tags
            string unformatedText = TagReplacer.Replace(SanitizeHtml(html).Replace(Environment.NewLine, ""), "");

            //remove xml comments
            var comments = new Regex("<!--(?s).*?-->");
            unformatedText = comments.Replace(unformatedText, string.Empty);

			if (preserveSpaces)
			{
                unformatedText = HttpUtility.HtmlDecode(BrReplacer.Replace(unformatedText, " "));
			}
        ...
        public static string ToPlainText(string html)
        {
            ...
            html = TagReplacer.Replace(SanitizeHtml(html).Replace(Environment.NewLine, ""), "");

    ...
    public class NotifyEngine : INotifyEngine
    {
        ...
        private void StyleMessage(NoticeMessage message)
        {
            try
            {
                ...
                {
                    var stylerType = Type.GetType(message.Pattern.Styler);
                    var styler = Activator.CreateInstance(stylerType) as IPatternStyler;
                    if (styler != null)
                    {
            ...
            {
                LogHolder.Log("ASC.Notify").Warn("error styling message", exc);
            }
        }

...
namespace ASC.Bookmarking.Dao
{
    public class BookmarkingHibernateDao : BookmarkingSessionObject<BookmarkingHibernateDao>
    {
        private DbManager DbManager
        {
            get
            {
                return DbManager.FromHttpContext(BookmarkingBusinessConstants.BookmarkingDbID);
            }
        }
            ...
            try
            {
                ...
                var bookmarks = DbManager.ExecuteList(
                        new SqlQuery()
                            ...
                            .ConvertAll(r => Converters.ToBookmark(r));
                return bookmarks;
            }
            catch
            {
            ...
            if (String.IsNullOrEmpty(startSymbols))
            {
                var allTags = DbManager.ExecuteList(
                    new SqlQuery()
                        ...
                        .Where("Tenant", Tenant)
                        .SetMaxResults(limit))
                        .ConvertAll(r => Converters.ToTag(r));
                return allTags;
            }
        ...
        internal void UpdateBookmark(UserBookmark userBookmark, IList<Tag> tags)
        {
            ...
            try
            {
                ...
                foreach (var tag in tags)
                {
                    ...
                    UserBookmarkTag ubt = new UserBookmarkTag() { UserBookmarkID = userBookmark.UserBookmarkID, TagID = tag.TagID };
        ...
        internal void UpdateBookmark(Bookmark bookmark, IList<Tag> tags)
        {
            ...
            try
            {
                var date = ASC.Core.Tenants.TenantUtil.DateTimeToUtc(bookmark.Date);
                ...
                foreach (var tag in tags)
                {
                    ...
                    BookmarkTag bt = new BookmarkTag() { BookmarkID = bookmark.ID, TagID = tag.TagID };

                    bt.BookmarkTagID = DbManager.ExecuteScalar<long>(
                            new SqlInsert("bookmarking_bookmarktag", true)
                            .InColumns("BookmarkID", "TagID", "Tenant")
                            .Values(bookmark.ID, tag.TagID, Tenant)
                            .Identity(0, 0L, true));

                    UserBookmarkTag ubt = new UserBookmarkTag() { UserBookmarkID = userBookmarkId, TagID = tag.TagID };
        ...
        internal IList<Bookmark> GetMostRecentBookmarksWithRaiting(int FirstResult, int MaxResults)
        {
            try
            {
                var bookmarks = DbManager.ExecuteList(
                                    ...
                                    .SetFirstResult(FirstResult)
                                    .SetMaxResults(MaxResults))
                                    .ConvertAll(r => Converters.ToBookmarkWithRaiting(r));
                return bookmarks;
            }
            catch
            {
        ...
        internal IList<Bookmark> GetBookmarksSortedByRaiting(DateTime timeInterval, int FirstResult, int MaxResults)
        {
            try
            {
                ...
                var bookmarks = DbManager.Connection.CreateCommand(@"select b.ID, b.URL, b.Date, b.Name, b.Description, b.UserCreatorID, count(b.ID) as r
from bookmarking_bookmark as b, bookmarking_userbookmark as ub
...
Group By b.ID Order By count(ub.BookmarkID) DESC, Date DESC limit @first, @max")
        ...
        internal IList<Tag> GetTagsSortByPopularity(int limit)
        {
            try
            {
                if (limit < 1)
                ...
                return GetMostPopularTags(0, limit);
            }
            catch
            {
                return null;
        ...
        internal IList<Tag> GetMostPopularTags(int FirstResult, int MaxResults)
        {
            try
            {
                var tags = DbManager.Connection.CreateCommand(@"select t.tagid, t.name, count(t.tagid) TagsCount " +
                    "from bookmarking_userbookmarktag u, bookmarking_tag t where u.Tenant = @Tenant and t.tagid = u.tagid and t.tenant = @Tenant " +
                    "group by 1 order by 3 desc limit @first, @max")
                    .AddParameter("Tenant", Tenant)
                    .AddParameter("first", FirstResult)
                    .AddParameter("max", MaxResults)
                    .ExecuteList()
                    .ConvertAll(r => Converters.ToTagWithRaiting(r));
                return tags;
            }
            catch
            {
...
group by t.TagID order by TagsCount desc limit @l")
        ...
        internal IList<Tag> GetMostRecentTags(int FirstResult, int MaxResults)
        {
            try
            {
                var tags = DbManager.Connection.CreateCommand(@"select t.TagID, t.Name, max(ub.DateAdded) as DateAdded 
from bookmarking_userbookmarktag as ubt, bookmarking_userbookmark as ub, bookmarking_tag as t
where ubt.Tenant = @Tenant and t.TagID = ubt.TagID and ub.UserBookmarkID = ubt.UserBookmarkID
group by t.TagID, t.Name order by DateAdded desc limit @first, @max")
                                           .AddParameter("first", FirstResult)
                                           .AddParameter("max", MaxResults)
                                           .AddParameter("tenant", Tenant)
                                           .ExecuteList()
        ...
        internal IList<Tag> GetTagsSortedByName(int FirstResult, int MaxResults)
        {
            try
            {
                var tags = DbManager.Connection.CreateCommand(@"select tagid, name from bookmarking_tag where tenant = @Tenant order by 2 limit @first, @max")
                                    .AddParameter("first", FirstResult)
                                    .AddParameter("max", MaxResults)
                                    .AddParameter("tenant", Tenant)
                                    .ExecuteList()
        ...
        internal int GetTagsCount()
        {
            var count = DbManager.Connection.CreateCommand(@"select count(distinct b.TagID) from bookmarking_userbookmarktag as b where b.Tenant = @Tenant")
                          .AddParameter("Tenant", Tenant)
                          .ExecuteScalar<int>();
            return count;
        }
        ...
        {
            var tags = DbManager.Connection.CreateCommand(@"select distinct allbtt.TagID, t.Name, (select count(*) from bookmarking_bookmarktag as a where a.TagID = allbtt.TagID) as Raiting
from bookmarking_bookmark as b
...
group by TagID order by t.Name asc limit @l")
        ...
        internal IList<Comment> GetBookmarkComments(Bookmark b)
        {
            var comments = DbManager.ExecuteList(
                                ...
                                .ConvertAll(r => Converters.ToComment(r));
        ...
        internal Comment AddComment(Comment comment)
        {
            try
            {
                var date = ASC.Core.Tenants.TenantUtil.DateTimeToUtc(comment.Datetime);
                DbManager.ExecuteNonQuery(
        ...
        internal IList<Bookmark> SearchBookmarks(IEnumerable<string> searchStringList, int FirstResult, int MaxResults, bool skipCountQuery)
        {
            try
            {
                ...
                if (searchStringList.Count() == 0) return new List<Bookmark>();
...
where b.Tenant = @Tenant and g.BookmarkID = b.ID and t.TagID = g.TagID ");
                ...
                var query = DbManager.Connection.CreateCommand("select distinct b.ID, b.URL, b.Date, b.Name, b.Description, b.UserCreatorID" + sb.ToString() + " limit @FirstResult, @MaxResults")
                                .AddParameter("Tenant", Tenant)
                                .AddParameter("FirstResult", FirstResult)
                                .AddParameter("MaxResults", MaxResults);

                var countQuery = DbManager.Connection.CreateCommand("select count(distinct b.ID)" + sb.ToString())
                                .AddParameter("Tenant", Tenant);
                ...
                var bookmakrs = query.ExecuteList().ConvertAll(r => Converters.ToBookmark(r));
                return bookmakrs;
            }
            catch
            {
...
order by b.Date desc ");
            ...
            try
            {
                var count = DbManager.Connection.CreateCommand(selectCountClause + sb.ToString())
                ...
                var bookmarks = DbManager.Connection.CreateCommand(selectClause + sb.ToString() + limitString)
                                        .AddParameter("tagName", searchString)
                                        .AddParameter("Tenant", Tenant)
                                        .AddParameter("first", FirstResult)
                                        .AddParameter("max", MaxResults)
...
group by  b.ID, t.TagID order by CountIds desc limit @FirstResult, @MaxResults", tagsString))
                ...
                var bookmarks = command.ExecuteList().ConvertAll(r => Converters.ToBookmarkWithRaiting(r));
        ...
        internal IList<UserBookmark> GetUserBookmarks(Bookmark b)
        {
            var userBookmarks = DbManager.ExecuteList(
                                    ...
                                    .ConvertAll(r => Converters.ToUserBookmark(r));
        ...
        internal UserBookmark GetCurrentUserBookmark(Bookmark b)
        {
            var userBookmark = DbManager.ExecuteList(
                                            ...
                                            .Where(Exp.Eq("UserID", GetCurrentUserId())
                                                   ...
                                                   & Exp.Eq("Tenant", Tenant)))
                                            .ConvertAll(r => Converters.ToUserBookmark(r));
            if (userBookmark != null && userBookmark.Count == 1)
            {
        ...
        internal Bookmark GetBookmarkWithUserBookmarks(string url)
        {
            var bookmarks = DbManager.ExecuteList(
                                            new SqlQuery()
                                            .Select("b.ID, b.URL, b.Date, b.Name, b.Description, b.UserCreatorID", "u.UserBookmarkID, u.UserID, u.DateAdded, u.Name, u.Description, u.BookmarkID, u.Raiting", "c.ID, c.UserID, c.Content, c.Datetime, c.Parent, c.BookmarkID, c.Inactive")
                                            .From("bookmarking_bookmark b")
                                            .LeftOuterJoin("bookmarking_userbookmark u", Exp.EqColumns("b.ID", "u.BookmarkID"))
                                            .LeftOuterJoin("bookmarking_comment c", Exp.EqColumns("b.ID", "c.BookmarkID"))
                                            .Where(Exp.Eq("b.URL", url) & Exp.Eq("b.Tenant", Tenant)))
                                            .ConvertAll(r => Converters.ToBookmarkWithUserBookmarks(r));
        ...
        internal void SetBookmarkTags(IList<Bookmark> bookmarks)
        {
            ...
            var tags = DbManager.ExecuteList(
                ...
                .ConvertAll(r => Converters.ToTagWithBookmarkID(r));

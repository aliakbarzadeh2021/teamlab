...
namespace ASC.Xmpp.Server.Services.Muc2.Room
{
	...
	using Settings;
	using Streams;
	using Item = agsXMPP.protocol.x.muc.Item;
	...
	internal class MucRoom : XmppServiceBase
	{
		...
		public void AdminCommand(IQ iq, MucRoomMember member)
		{
			...
			if (admin != null)
			{
				...
				{
					foreach (agsXMPP.protocol.x.muc.iq.admin.Item item in admin.GetItems())
					{
						if (item.Actor == null && item.Affiliation != Affiliation.none)
						{
							...
							foreach (MucRoomMemberInfo mucRoomMember in RoomSettings.Members)
							{
								...
								{
									returnAdmin.AddItem(
										new agsXMPP.protocol.x.muc.iq.admin.Item(mucRoomMember.Affiliation,
																		 mucRoomMember.Role,
																		 mucRoomMember.Jid));
								}
		...
		private MucRoomMemberInfo FindMemberInfo(agsXMPP.protocol.x.muc.iq.admin.Item item)
		{

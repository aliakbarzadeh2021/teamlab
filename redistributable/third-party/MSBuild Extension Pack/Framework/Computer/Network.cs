...
namespace MSBuild.ExtensionPack.Computer
{
	...
	public class Network : BaseTask
	{
		...
		private void GetInternalIP()
		{
			...
			var IPList = new List<ITaskItem>();
			for (int i = 0; i < hostEntry.AddressList.Length; i++)
			{
				var ipString = hostEntry.AddressList[i].ToString();
				var ipAddress = IPAddress.None;
                if (IPAddress.TryParse(ipString, out ipAddress) && ipAddress.AddressFamily == AddressFamily.InterNetwork)
				{
					IPList.Add(new TaskItem(ipString));
				}
			}
			this.IP = IPList.ToArray();
		}

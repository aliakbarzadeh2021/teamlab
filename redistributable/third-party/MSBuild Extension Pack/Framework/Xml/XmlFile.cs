...
namespace MSBuild.ExtensionPack.Xml
{
    ...
    public class XmlFile : BaseTask
    {
        ...
        private void AddAttribute()
        {
            ...
            XmlNode elementNode = SelectSingleNode(this.Element);
            if (elementNode == null)
            {
                Log.LogWarning(string.Format(CultureInfo.CurrentUICulture, "Element not found: {0}", this.Element));
                return;
            }
        ...
        private void AddElement()
        {
            ...
            XmlNode parentNode = SelectSingleNode(this.ParentElement);
            if (parentNode == null)
            {
                Log.LogWarning("ParentElement not found: " + this.ParentElement);
                return;
            }

            // Ensure node does not already exist
            XmlNode newNode = SelectSingleNode(this.ParentElement + "/" + this.Element);
            if (newNode == null)
            {
        ...
        private void RemoveElement()
        {
            ...
            XmlNode nodeToRemove = SelectSingleNode(this.ParentElement + "/" + this.Element);
